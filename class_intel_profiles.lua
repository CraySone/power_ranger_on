...
function ClassIntelProfiles.Ensure(settings)
    ...
    if not settings.classIntelEditProfile then settings.classIntelEditProfile = "melee" end
end
...
function ClassIntelProfiles.Classify(targetInfo, className)
    ...
    if hasTree(targetInfo, 3) then return "mage" end
...
function ClassIntelProfiles.Label(profileKey)
    return PROFILE_LABELS[profileKey] or "Melee"
end

function ClassIntelProfiles.ShouldShow(settings, targetInfo, className, statKey)
    if settings.classIntelEnabled ~= true then return nil end
...
function ClassIntelProfiles.CycleEditProfile(settings, delta)
    ClassIntelProfiles.Ensure(settings)
    local current = settings.classIntelEditProfile or "melee"
    local index = 1
...
function ClassIntelProfiles.ToggleStat(settings, statKey)
    ClassIntelProfiles.Ensure(settings)
    local profile = settings.classIntelProfiles[settings.classIntelEditProfile or "melee"]
